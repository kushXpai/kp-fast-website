...
// Alternative export function for create tab data
export const exportCreateTabToPDF = (
  ...
  doc.text(`Test Date: ${formattedDate}`, 20, 45);
  doc.text(`Total Players: ${players.length}`, 20, 55);

  // Prepare table data from current state
  const tableColumns = [
    'Player Name',
    'Long Jump (m)',
    '10m Sprint (s)',
  ...
  const tableRows = players.map(player => [
    player.name,
    testResults[player.id]?.['long_jump_m_average'] || '-',
    testResults[player.id]?.['ten_m_sprint_s_average'] || '-',
    testResults[player.id]?.['twenty_m_sprint_s_average'] || '-',
    testResults[player.id]?.['forty_m_sprint_s_average'] || '-',
    testResults[player.id]?.['run_a_three_s_average'] || '-',
    testResults[player.id]?.['yo_yo_test_level_average'] || '-'
  ]);
